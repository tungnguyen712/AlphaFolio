...
function verdictSignal(verdict: string): string {
  ...
  if (w === "BUY") return "bg-green-100 text-green-800";
  if (w === "SELL") return "bg-red-100 text-red-800";
...
export default function RecommendationsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div>
      <h3 className="mb-4 text-sm font-semibold text-neutral-800">Recommendations</h3>
      {recs.length === 0 ? (
        <EmptyState
