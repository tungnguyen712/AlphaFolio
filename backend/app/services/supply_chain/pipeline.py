"""Supply chain pipeline — orchestrates Wikidata + 10-K + Tavily into SupplyChainReport.

No LangGraph: three parallel async fetches then merge. The result is fast
enough to serve synchronously (<8s cold, <200ms cached).
"""
...
async def _cached_tavily_extract(ticker: str, snippets: list[dict]) -> SupplyChainEntities:
    key = make_cache_key("sc.tavily.haiku.v3", ticker=ticker.upper())
    cached = await cache_get(key)
    if cached is not None:
        return SupplyChainEntities.model_validate(cached)
...
async def run_supply_chain(ticker: str) -> SupplyChainReport:
    """Orchestrate Wikidata + 10-K + Tavily → SupplyChainReport."""
    upper = ticker.upper()
    ...
    if unresolved_high:
        resolved = await asyncio.gather(*[_try_resolve_ticker(r.name) for r in unresolved_high])
        name_to_ticker = {
            r.name: t for r, t in zip(unresolved_high, resolved) if t
        }
        all_rels = [
            r.model_copy(update={
